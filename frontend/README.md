...
这是 AI API Test Platform / AI 接口自动化测试平台的前端工程。当前已完成 Vue3 前端业务页面，并在第 9 阶段增强接口导入页，在第 10 阶段增强环境配置页和执行报告页，在第 10.1 阶段增加断言系统 2.0 的展示与 DSL 可视化编辑入口。
...
第 11 阶段后，AI 用例生成由后端真实大模型接口完成，前端仍通过既有后端 API 触发。
...
- AI 断言建议不决定测试结果，只有插入到用户断言 DSL 后才参与执行。
- 真实 AI 大模型由后端统一接入，前端不直接调用模型服务。
